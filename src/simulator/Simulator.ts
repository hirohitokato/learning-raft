...
import { RaftNode, Message } from "./../nodes/RaftNode";
...
export class Simulator {
    ...
    constructor(nodeCount: number) {
        for (let i = 0; i < nodeCount; i++) {
            ...
            const node = new RaftNode(i, peers, this.clock, (message: Message) => {
                this.send(message)
            }, Math.floor(Math.random() * 150) + 150) // 150ms〜300msの範囲でランダムに選挙タイムアウトを設定
